...
class IdentifiableEntity(object): #Rubens
    def __init__(self, id: str):
        if not isinstance(id, str):
            raise ValueError("IdentifiableEntity's id must be a string!")
        self.id = id
...
class Person(IdentifiableEntity): #Rubens
    def __init__(self, id: str, name: str):
        ...
        if not isinstance(name, str):
            raise ValueError("Person's name must be a string!")
...
class CulturalHeritageObject(IdentifiableEntity): #Ben
    def __init__(
        ...
        if not isinstance(title, str):
            raise ValueError("CulturalHeritageObject's title must be a string!")
        if not isinstance(owner, str):
            raise ValueError("CulturalHeritageObject's owner must be a string!")
        if not isinstance(place, str):
            raise ValueError("CulturalHeritageObject's place must be a string!")
        if (not isinstance(date, str)) and date is not None:
            raise ValueError("CulturalHeritageObject's date must be a string or None!")
        if (
            not isinstance(hasAuthor, Person)
            and not isinstance(hasAuthor, list)
            and hasAuthor is not None
        ):
            raise ValueError(
                "CulturalHeritageObject's author must be a list or a string or None!"
            )
...
class Activity(object): #Rubens
    def __init__(
        ...
        if not isinstance(refersTo, CulturalHeritageObject):
            raise ValueError("Activity's refersTo must be a CulturalHeritageObject!")
        if not isinstance(institute, str):
            raise ValueError("Activity's institute must be a string!")
        if not isinstance(person, str) and person is not None:
            raise ValueError("Activity's person must be a string or None!")
        if not isinstance(start, str) and start is not None:
            raise ValueError("Activity's start must be a string or None!")
        if not isinstance(end, str) and end is not None:
            raise ValueError("Activity's end must be a string or None!")
...
class Acquisition(Activity):
    def __init__(
        ...
        if not isinstance(refersTo, CulturalHeritageObject):
            print("Invalid refersTo object type:", type(refersTo))
            raise ValueError(
                "Acquisition's refersTo must be an instance of CulturalHeritageObject!"
            )

        super().__init__(refersTo, institute, person, tool, start, end)
        if not isinstance(technique, str):
            raise ValueError("Acquisition's technique must be a string!")
...
class Handler(object): #Ekaterina
    ...
    def setDbPathOrUrl(self, value: str) -> bool:
...
class ProcessDataQueryHandler(QueryHandler):
    ...
    def getActivitiesByResponsibleInstitution(self, institution_str: str) -> pd.DataFrame: #Ekaterina
        db_file = "json.db"
        try:
            conn = sqlite3.connect(db_file)
    ...
    def getActivitiesUsingTool(self, tool_str: str) -> pd.DataFrame: #Rubens
        ...
        try:
            ...
            query = """
                SELECT object_id, responsible_institute, responsible_person, technique, tool, start_date, end_date, 'Acquisition' as type  FROM Acquisition WHERE tool LIKE ?
                UNION
                SELECT object_id, responsible_institute, responsible_person, NULL as technique, tool, start_date, end_date, 'Processing' as type  FROM Processing WHERE tool LIKE ?
                UNION
                SELECT object_id, responsible_institute, responsible_person, NULL as technique, tool, start_date, end_date, 'Modelling' as type  FROM Modelling WHERE tool LIKE ?
                UNION
                SELECT object_id, responsible_institute, responsible_person, NULL as technique, tool, start_date, end_date, 'Optimising' as type  FROM Optimising WHERE tool LIKE ?
                UNION
                SELECT object_id, responsible_institute, responsible_person, NULL as technique, tool, start_date, end_date, 'Exporting' as type  FROM Exporting WHERE tool LIKE ?
            """
    ...
    def getActivitiesStartedAfter(self, start_date: str) -> pd.DataFrame: #Amanda
        ...
        try:
            conn = sqlite3.connect(db_file)
            query = """
            SELECT object_id, responsible_institute, responsible_person, technique, tool, start_date, end_date, 'Acquisition' as type 
            FROM Acquisition WHERE start_date >= ? 
            UNION 
            SELECT object_id, responsible_institute, responsible_person, NULL as technique, tool, start_date, end_date, 'Processing' as type 
            FROM Processing WHERE start_date >= ? 
            UNION 
            SELECT object_id, responsible_institute, responsible_person, NULL as technique, tool, start_date, end_date, 'Modelling' as type 
            FROM Modelling WHERE start_date >= ? 
            UNION 
            SELECT object_id, responsible_institute, responsible_person, NULL as technique, tool, start_date, end_date, 'Optimising' as type 
            FROM Optimising WHERE start_date >= ? 
            UNION 
            SELECT object_id, responsible_institute, responsible_person, NULL as technique, tool, start_date, end_date, 'Exporting' as type 
    ...
    def getActivitiesEndedBefore(self, end_date: str) -> pd.DataFrame: #Amanda
        ...
        try:
            ...
            query = (
                "SELECT object_id, responsible_institute, responsible_person, technique, tool, start_date, end_date, 'Acquisition' as type FROM Acquisition WHERE end_date <= ? UNION "
                "SELECT object_id, responsible_institute, responsible_person, NULL as technique, tool, start_date, end_date, 'Processing' as type FROM Processing WHERE end_date <= ? UNION "
                "SELECT object_id, responsible_institute, responsible_person, NULL as technique, tool, start_date, end_date, 'Modelling' as type FROM Modelling WHERE end_date <= ? UNION "
                "SELECT object_id, responsible_institute, responsible_person, NULL as technique, tool, start_date, end_date, 'Optimising' as type  FROM Optimising WHERE end_date <= ? UNION "
                "SELECT object_id, responsible_institute, responsible_person, NULL as technique, tool, start_date, end_date, 'Exporting' as type FROM Exporting WHERE end_date <= ?"
            )
...
class BasicMashup(object):
    ...
    def getAllCulturalHeritageObjects(self) -> List[CulturalHeritageObject]: #Ekaterina
        objects_list = []
        df = pd.DataFrame()
    ...
    def getAuthorsOfCulturalHeritageObject(self, object_id: str) -> List[Person]: #Ekaterina
        authors_list = []
    ...
    def getActivitiesByResponsiblePerson(self, person_name: str) -> List[Activity]: #Ben/Ekaterina
        all_activities = []
        activities_df = pd.DataFrame()
    ...
    def getActivitiesStartedAfter(self, date: str) -> List[Activity]: #Amanda/Ekaterina
        all_activities = []
        activities_df = pd.DataFrame()
...
class AdvancedMashup(BasicMashup):
    ...
    def getActivitiesOnObjectsAuthoredBy(self, author_id: str) -> list[Activity]: #Rubens
        ...
        selected_rows = all_activities[all_activities['object_id'].isin(related_ids_str)]
    ...
    def getAuthorsOfObjectsAcquiredInTimeFrame(self, start_date: str, end_date: str) -> list[Person]: #Rubens
        acquired_authors = []

        activities_started = self.processQuery[0].getActivitiesStartedAfter(start_date)
        started_ids = set(activities_started[activities_started['type'] == 'Acquisition']['object_id'])

        activities_ended = self.processQuery[0].getActivitiesEndedBefore(end_date)
        ended_ids = set(activities_ended[activities_ended['type'] == 'Exporting']['object_id'])
